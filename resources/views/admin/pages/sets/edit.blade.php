...
@section('main-content')
    ...
    <div class="form-card">
        ...
        <div class="form-body">
            ...
            <form action="{{ route('admin.sets.update', $set) }}" method="POST" class="category-form" id="set-form" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" id="name" name="name" class="custom-input {{ $errors->has('name') ? 'input-error' : '' }}" value="{{ old('name', $set->name) }}" required>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select id="type" name="type" class="custom-input {{ $errors->has('type') ? 'input-error' : '' }}" required>
                                    ...
                                    <option value="{{ \App\Models\Set::TYPE_FREE }}" {{ old('type', $set->type) === \App\Models\Set::TYPE_FREE ? 'selected' : '' }}>Miễn phí</option>
                                    <option value="{{ \App\Models\Set::TYPE_PREMIUM }}" {{ old('type', $set->type) === \App\Models\Set::TYPE_PREMIUM ? 'selected' : '' }}>Premium</option>
                                </select>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="number" step="0.01" id="size" name="size" class="custom-input {{ $errors->has('size') ? 'input-error' : '' }}" value="{{ old('size', $set->size) }}" required>
                                <div class="error-message" id="error-size">@error('size') {{ $message }} @enderror</div>
                            </div>
                        </div>
                        <div class="col-md-6" id="price-field" style="display: {{ old('type', $set->type) === 'premium' ? 'block' : 'none' }};">
                            <div class="form-group">
                                <label for="price" class="form-label-custom">Giá (Xu) <span class="required-mark">*</span></label>
                                <input type="number" id="price" name="price" class="custom-input {{ $errors->has('price') ? 'input-error' : '' }}" value="{{ old('price', $set->price) }}">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                <div class="checkbox-wrapper">
                                    <input type="checkbox" id="is_featured" name="is_featured" value="1" {{ old('is_featured', $set->is_featured) ? 'checked' : '' }}>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <div id="logo-preview" class="mt-2">
                                    @if($set->image)
                                        <img src="{{ Storage::url($set->image) }}" alt="Current logo" style="max-width: 200px; max-height: 200px; border-radius: 8px; border: 1px solid #ddd;">
                                    @endif
                                </div>
                            </div>
                        </div>
                                ...
                                <div id="photo-preview" class="mt-2" style="display:flex;flex-wrap:wrap;gap:10px;">
                                    @if($set->photos && $set->photos->count())
                                        @foreach($set->photos as $photo)
                                            <div class="existing-photo" data-photo-id="{{ $photo->id }}" style="position: relative; display: inline-block; margin: 5px;">
                                                <img src="{{ Storage::url($photo->path) }}" alt="Existing photo" style="width: 80px; height: 80px; object-fit: cover; border-radius: 6px; border: 1px solid #ddd;">
                                                <button type="button" class="remove-existing-photo" data-photo-id="{{ $photo->id }}" style="position: absolute; top: -5px; right: -5px; width: 20px; height: 20px; border-radius: 50%; background: #dc3545; color: white; border: none; font-size: 12px; cursor: pointer; display: flex; align-items: center; justify-content: center;">×</button>
                                            </div>
                                        @endforeach
                                    @endif
                                </div>
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <label class="d-flex align-items-center" style="gap:6px;">
                                    <input type="checkbox" name="status" value="1" {{ old('status', $set->status) ? 'checked' : '' }}> Kích hoạt
                                </label>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="chip-select" data-select-id="album_ids" style="position:relative;">
                                    ...
                                    <div class="chip-select-dropdown" style="position:absolute;left:0;right:0;z-index:20;background:#fff;border:1px solid #e9ecef;border-radius:6px;display:none;max-height:220px;overflow:auto;">
                                        @foreach($albums as $al)
                                            <label style="display:flex;align-items:center;gap:8px;padding:8px 12px;cursor:pointer;">
                                                <input type="checkbox" value="{{ $al->id }}" data-text="{{ $al->name }}" {{ in_array($al->id, $selectedAlbums) ? 'checked' : '' }}>
                                                <span>{{ $al->name }}</span>
                                            </label>
                                        @endforeach
                                    </div>
                                    ...
                                    <select id="album_ids" name="album_ids[]" multiple style="display:none;">
                                        @foreach($albums as $al)
                                            <option value="{{ $al->id }}" {{ in_array($al->id, $selectedAlbums) ? 'selected' : '' }}>{{ $al->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="chip-select" data-select-id="category_ids" style="position:relative;">
                                    ...
                                    <div class="chip-select-dropdown" style="position:absolute;left:0;right:0;z-index:20;background:#fff;border:1px solid #e9ecef;border-radius:6px;display:none;max-height:220px;overflow:auto;">
                                        @foreach($categories as $ct)
                                            <label style="display:flex;align-items:center;gap:8px;padding:8px 12px;cursor:pointer;">
                                                <input type="checkbox" value="{{ $ct->id }}" data-text="{{ $ct->name }}" {{ in_array($ct->id, $selectedCategories) ? 'checked' : '' }}>
                                                <span>{{ $ct->name }}</span>
                                            </label>
                                        @endforeach
                                    </div>
                                    ...
                                    <select id="category_ids" name="category_ids[]" multiple style="display:none;">
                                        @foreach($categories as $ct)
                                            <option value="{{ $ct->id }}" {{ in_array($ct->id, $selectedCategories) ? 'selected' : '' }}>{{ $ct->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="chip-select" data-select-id="color_ids" style="position:relative;">
                                    ...
                                    <div class="chip-select-dropdown" style="position:absolute;left:0;right:0;z-index:20;background:#fff;border:1px solid #e9ecef;border-radius:6px;display:none;max-height:220px;overflow:auto;">
                                        @foreach($colors as $cl)
                                            <label style="display:flex;align-items:center;gap:8px;padding:8px 12px;cursor:pointer;">
                                                <input type="checkbox" value="{{ $cl->id }}" data-text="{{ $cl->name }}" {{ in_array($cl->id, $selectedColors) ? 'checked' : '' }}>
                                    ...
                                    <select id="color_ids" name="color_ids[]" multiple style="display:none;">
                                        @foreach($colors as $cl)
                                            <option value="{{ $cl->id }}" {{ in_array($cl->id, $selectedColors) ? 'selected' : '' }}>{{ $cl->name }} ({{ $cl->value }})</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="chip-select" data-select-id="software_ids" style="position:relative;">
                                    ...
                                    <div class="chip-select-dropdown" style="position:absolute;left:0;right:0;z-index:20;background:#fff;border:1px solid #e9ecef;border-radius:6px;display:none;max-height:220px;overflow:auto;">
                                        @foreach($software as $sw)
                                            <label style="display:flex;align-items:center;gap:8px;padding:8px 12px;cursor:pointer;">
                                                <input type="checkbox" value="{{ $sw->id }}" data-text="{{ $sw->name }}" {{ in_array($sw->id, $selectedSoftware) ? 'checked' : '' }}>
                                                <span>{{ $sw->name }}</span>
                                            </label>
                                        @endforeach
                                    </div>
                                    ...
                                    <select id="software_ids" name="software_ids[]" multiple style="display:none;">
                                        @foreach($software as $sw)
                                            <option value="{{ $sw->id }}" {{ in_array($sw->id, $selectedSoftware) ? 'selected' : '' }}>{{ $sw->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="chip-select" data-select-id="tag_ids" style="position:relative;">
                                    ...
                                    <div class="chip-select-dropdown" style="position:absolute;left:0;right:0;z-index:20;background:#fff;border:1px solid #e9ecef;border-radius:6px;display:none;max-height:220px;overflow:auto;">
                                        @foreach($tags as $tg)
                                            <label style="display:flex;align-items:center;gap:8px;padding:8px 12px;cursor:pointer;">
                                                <input type="checkbox" value="{{ $tg->id }}" data-text="{{ $tg->name }}" {{ in_array($tg->id, $selectedTags) ? 'checked' : '' }}>
                                                <span>{{ $tg->name }}</span>
                                            </label>
                                        @endforeach
                                    </div>
                                    ...
                                    <select id="tag_ids" name="tag_ids[]" multiple style="display:none;">
                                        @foreach($tags as $tg)
                                            <option value="{{ $tg->id }}" {{ in_array($tg->id, $selectedTags) ? 'selected' : '' }}>{{ $tg->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
