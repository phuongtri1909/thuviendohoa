                ...
                <form action="{{ route('admin.blogs.update', $blog) }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="form-tabs">
                        ...
                        <div class="form-group">
                            ...
                            <div class="chip-select" data-select-id="tag_ids" style="position:relative;">
                                ...
                                <div class="chip-select-dropdown" style="position:absolute;left:0;right:0;z-index:20;background:#fff;border:1px solid #e9ecef;border-radius:6px;display:none;max-height:220px;overflow:auto;">
                                    @foreach($tags as $tag)
                                        <label style="display:flex;align-items:center;gap:8px;padding:8px 12px;cursor:pointer;">
                                            <input type="checkbox" value="{{ $tag->id }}" data-text="{{ $tag->name }}" {{ in_array($tag->id, old('tag_ids', $selectedTags)) ? 'checked' : '' }}>
                                            <span>{{ $tag->name }}</span>
                                        </label>
                                    @endforeach
                                </div>
                                ...
                                <select id="tag_ids" name="tag_ids[]" multiple style="display:none;">
                                    @foreach($tags as $tag)
                                        <option value="{{ $tag->id }}" {{ in_array($tag->id, old('tag_ids', $selectedTags)) ? 'selected' : '' }}>{{ $tag->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            @if($blog->image)
                                <div class="current-image">
                                    <img src="{{ asset('storage/' . $blog->image) }}" alt="Current image" style="max-width: 200px; margin-bottom: 10px;">
                                </div>
                            @endif
                            <input type="file" id="image" name="image"
                                class="custom-input {{ $errors->has('image') ? 'input-error' : '' }}"
                                accept="image/jpeg,image/png,image/jpg,image/gif,image/webp">
                            <small class="form-text">Định dạng: jpeg, png, jpg, gif, webp. Tối đa 10MB. Để trống nếu không muốn thay đổi.</small>
                        ...
                        <div class="form-group">
                            ...
                            @if($blog->image_left)
                                <div class="current-image">
                                    <img src="{{ asset('storage/' . $blog->image_left) }}" alt="Current left image" style="max-width: 200px; margin-bottom: 10px;">
                                </div>
                            @endif
